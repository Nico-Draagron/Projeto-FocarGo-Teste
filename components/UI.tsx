...
export const Card: React.FC<CardProps> = ({ children, className = "", ...props }) => (
  <div 
    className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 animate-in fade-in ${props.onClick ? 'cursor-pointer' : ''} ${className}`}
    {...props}
  >
    {children}
  </div>
...
export const Badge = ({ level, icon }: { level: number | string, icon?: string }) => (
  <div className="inline-flex items-center gap-2 bg-purple/10 border-2 border-purple/20 rounded-full px-3 py-1 animate-in slide-in-from-bottom-2">
